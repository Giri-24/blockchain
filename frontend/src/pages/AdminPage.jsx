...
export default function AdminPage() {
  ...
  if (!account) return (
    <div className="min-h-screen bg-cyber-dark pt-20 flex items-center justify-center">
      <p className="text-gray-400 font-mono">Connect wallet to access admin panel.</p>
    </div>
  );
  ...
  if (!isOwner) return (
    <div className="min-h-screen bg-cyber-dark pt-20 flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-gray-400 font-mono">This panel is restricted to the contract owner.</p>
        <p className="font-mono text-xs text-gray-600 mt-2">Your address: {account}</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-cyber-dark bg-grid-pattern bg-grid pt-20">
      <div className="max-w-6xl mx-auto px-4 py-10">
        <div className="flex items-center gap-3 mb-2">
          <span className="px-2.5 py-1 font-mono text-xs bg-purple-500/20 text-purple-400 border border-purple-500/40 rounded">ADMIN</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: "Total Jobs",  value: allJobs.length,    color: "text-white" },
            { label: "Flagged",     value: suspicious.length, color: "text-red-400" },
            { label: "Active",      value: allJobs.filter(j => !j.isSuspicious).length, color: "text-cyber-green" },
            { label: "Owner",       value: account?.slice(0,6)+"...", color: "text-purple-400" },
          ].map(s => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="p-6 rounded-xl bg-cyber-card border border-cyber-border">
            <h2 className="font-display text-base font-semibold text-white uppercase tracking-wide mb-4">
              ✅ Verify Recruiter
            </h2>
            <form onSubmit={handleVerify} className="space-y-3">
              <input value={recruiterAddr} onChange={e => setRecruiterAddr(e.target.value)}
                placeholder="0x... wallet address"
                className="w-full px-3 py-2.5 bg-white/5 border border-cyber-border text-white font-mono text-sm rounded-lg placeholder-gray-600 focus:outline-none focus:border-cyber-green/50" />
          ...
          <div className="p-6 rounded-xl bg-cyber-card border border-red-500/20">
            ...
            <form onSubmit={handleSlash} className="space-y-3">
              <input value={slashAddr} onChange={e => setSlashAddr(e.target.value)}
                placeholder="0x... wallet address"
                className="w-full px-3 py-2.5 bg-white/5 border border-red-500/30 text-white font-mono text-sm rounded-lg placeholder-gray-600 focus:outline-none focus:border-red-500/50" />
          ...
          <div className="p-6 rounded-xl bg-cyber-card border border-cyber-border lg:col-span-2">
            ...
            <div className="flex gap-3">
              <input value={checkAddr} onChange={e => setCheckAddr(e.target.value)}
                ...
                className="flex-1 px-3 py-2.5 bg-white/5 border border-cyber-border text-white font-mono text-sm rounded-lg placeholder-gray-600 focus:outline-none focus:border-cyber-blue/50" />
            ...
            {checkResult && (
              <div className="mt-4 p-4 bg-white/5 rounded-lg grid grid-cols-2 md:grid-cols-5 gap-3">
                {[
                  ...
                  <div key={r.label} className="text-center">
                    <div className={`font-mono text-sm font-bold ${r.color || "text-white"}`}>{r.value}</div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display text-lg font-semibold text-white">All Jobs Monitor</h2>
              ...
              {allJobs.map(job => (
                <div key={job.id}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-mono text-sm text-white">{job.ipfsData?.title || "Untitled"}</p>
                      <p className="font-mono text-xs text-gray-500">{job.recruiter?.slice(0, 14)}...</p>
                    </div>
                    {job.isSuspicious && (
